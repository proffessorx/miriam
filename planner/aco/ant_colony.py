...
class ant_colony:
	class ant(Thread):
		def __init__(self, init_location, possible_locations, pheromone_map, distance_callback, alpha, beta, first_pass=False):
			...
			alpha -> a parameter from the ACO algorithm to control the influence of the amount of pheromone when making a choice in pick_path()
			beta -> a parameters from ACO that controls the influence of the distance to the next node in pick_path()
		...
		def run(self):
			...
			until self.possible_locations is empty (the ant has visited all nodes)
				...
				_traverse() to:
					route_update() (to show latest traversal)
		...
		def _traverse(self, start, end):
			"""
			route_update() to show new traversal
